(() => {
  const resourceName = (window.GetParentResourceName && window.GetParentResourceName()) || 'mnc-handuiPlate';
  ...
  const plateBanner = el('plateBanner');
  const plateLabel = el('plateLabel');
  const plateCode = el('plateCode');
  const plateBadge = el('plateBadge');
  const plateMeta = el('plateMeta');
  ...
  function renderPlateBanner() {
    plateLabel.textContent = state.currentLabel;
    plateCode.textContent = state.currentPlate;
    if (state.hasOverride) {
      plateBadge.textContent = 'Custom';
      plateBadge.classList.add('custom');
      const when = state.updatedAt ? new Date(state.updatedAt).toLocaleString() : '';
      const modelBit = state.currentModel ? `Model: ${state.currentModel}` : '';
      const byBit = state.updatedBy ? `Last edited by ${state.updatedBy}${when ? ' · ' + when : ''}` : '';
      plateMeta.textContent = [modelBit, byBit].filter(Boolean).join(' · ');
    } else {
      plateBadge.textContent = 'Default';
      plateBadge.classList.remove('custom');
      const modelBit = state.currentModel ? `Model: ${state.currentModel}` : '';
      plateMeta.textContent = [modelBit, 'No overrides saved yet'].filter(Boolean).join(' · ');
    }
  }
  ...
  function renderPresetBar() {
    ...
    if (!state.currentPlate || presets.length === 0) {
  ...
  function renderSavedTable(filter) {
    ...
    const rows = savedListCache.filter((r) => {
      if (!q) return true;
    ...
    savedTableBody.innerHTML = rows
      .map((r) => {
        ...
        return `
          <tr>
            <td class="plate-code">${escapeAttr(r.plate)}</td>
            <td class="vehicle-name">${escapeAttr(r.model || '—')}</td>
            <td class="meta">${escapeAttr(r.updatedBy || '—')}</td>
            <td class="meta">${when}</td>
            <td>
              <div class="row-actions">
                <button data-revert="${escapeAttr(r.plate)}" class="danger">Revert</button>
              </div>
            </td>
          </tr>`;
      })
    ...
    savedTableBody.querySelectorAll('[data-revert]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const plate = btn.dataset.revert;
        const ok = await askConfirm(`Revert plate ${plate} back to its default handling? This deletes the saved override.`);
        if (!ok) return;
        await nuiPost('handui:deleteOverride', { plate });
        loadSavedOverrides();
      });
    });
  }
