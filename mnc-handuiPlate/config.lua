...
-- NOTE: unlike mnc-handui, overrides in this resource are saved per vehicle
-- PLATE, not per model. Two Sultans with different plates can have totally
-- different handling; editing one never touches the other. A side effect is
-- that changing a vehicle's plate (e.g. via a mod shop) orphans its saved
-- override, since the save is keyed to the plate string, not the vehicle.
