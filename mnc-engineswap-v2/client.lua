                ...
                for _, engine in ipairs(category.engines) do
                    table.insert(engineOptions, {
                        ...
                        onSelect = function()
                            ...
                            -- Broadcast to all clients so bystanders hear the sound
                            TriggerServerEvent('mnc-engineswap:broadcastEngineSound', plate, engine.sound)
                            -- ALWAYS save
    ...
    if progressSuccess and orderedEngine and orderedEngine.sound then
        ...
        if not DoesEntityExist(currentVehicle) or GetVehicleEngineHealth(currentVehicle) <= 0 then
            ...
            if soundSuccess and handlingSuccess then
                ...
                -- Broadcast the new sound to all connected clients so nearby
                -- players hear the engine change without needing to re-enter the car.
                TriggerServerEvent('mnc-engineswap:broadcastEngineSound', trackedVehiclePlate, orderedEngine.sound)
            else
                if Config.Debug then
...
-- =====================================================================
-- ENGINE SOUND BROADCAST RECEIVER
-- Fires on every client when any player completes an engine swap so that
-- bystanders also hear the new sound without needing to re-enter the car.
-- =====================================================================
RegisterNetEvent('mnc-engineswap:applyEngineSound', function(plate, sound)
    if not plate or not sound then return end
    plate = string.upper(plate:match("^%s*(.-)%s*$"))

    -- Search nearby vehicles for a matching plate
    local ped      = PlayerPedId()
    ...
    for _, veh in ipairs(vehicles) do
        if DoesEntityExist(veh) then
            local vehPlate = string.upper((GetVehicleNumberPlateText(veh) or ""):match("^%s*(.-)%s*$"))
            if vehPlate == plate then
                -- Only apply if the vehicle is within a reasonable distance
                local dist = #(pedPos - GetEntityCoords(veh))
                if dist < 200.0 then
                    SetVehicleEngineOn(veh, true, false, true)
                    pcall(function() ForceVehicleEngineAudio(veh, sound) end)
                    if Config.Debug then
                        print(("^2[mnc-engineswap]^7 applyEngineSound: applied '%s' to plate '%s' (dist: %.1f)"):format(sound, plate, dist))
                    end
                end
                break
            end
