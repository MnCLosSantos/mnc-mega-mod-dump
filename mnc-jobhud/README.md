...
# then copy the `mnc-jobhud/` folder into your server's resources directory

# OR download the ZIP from https://github.com/MnCLosSantos/mnc-mega-mod-dump/releases and extract just the `mnc-jobhud/` folder
```
...
Built on **QBCore Framework**, **ox_lib**, **oxmysql**, and **Font Awesome** icons — thanks to their communities and to everyone who filed bug reports and suggestions.
...
1. **Database**: Requires oxmysql — the `mnc_hud_styles` table is created automatically on first start
2. **Compatibility**: QBCore only — not compatible with ESX
3. **Legal**: For use on FiveM servers only, respect Rockstar's ToS

---

**Track your job, bank, and cash in style. 💼**
